...
def socializar_en_feed():
    ...
    comentario = obtener_respuesta_ia(
        f"Comenta este post con ironía académica (máx 2 frases): '{target.get('content', '')[:200]}'"
    )
    if comentario:
        api_moltbook("POST", f"/posts/{target.get('id')}/comments", {"content": comentario})
...
def comandos_control(message):
    ...
    if cmd == 'publicar':
        ...
        bot.send_message(message.chat.id, "🟢 Nova: Sistema estable.")
